...
function v5() {
  ...
  for (let context of contexts) {
    appsColl.insert({
      ...
      wsProvider:
        context["wsProvider"] ||
        "wss://mainnet.infura.io/ws/v3/36e48f8228ad42a297049cabc1101324",
    });
...
function v5_5() {
  ...
  console.log(
    "removing 'score', 'verifications', 'flaggers', 'trusted' attributes form users collection"
  );
  const usersColl = arango._collection("users");
  query`
  ...
  console.log(
    "removing 'Yekta_0', 'Yekta_1', 'Yekta_2', 'Yekta_3', 'Yekta_4', 'Yekta_5' documents form verifications collection"
  );
  const verificationsColl = arango._collection("verifications");
  for (let verificationName of [
    "Yekta_0",
    "Yekta_1",
    "Yekta_2",
    "Yekta_3",
...
function v5_6_1() {
  ...
  for (let group of groups) {
    groupsColl.update(group, {
      ...
      admins: (group.admins || group.founders).map((a) =>
        a.replace("users/", "")
      ),
    });
  }
...
function v5_9() {
  console.log(
    "reducing 'recovery' level to 'just met' for connections that another side is not 'already known' or 'recovery'"
  );
  const connectionsColl = arango._collection("connections");
  const connectionsHistoryColl = arango._collection("connectionsHistory");
  const now = Date.now();
...
function v5_9_2() {
  let hashes = variablesColl.document("VERIFICATIONS_HASHES").hashes;
  variablesColl.update("VERIFICATIONS_HASHES", {
  ...
  contextsColl
    ...
    .map((context) => {
      ...
      for (let doc of docs) {
        if (
          !doc.contextId ||
          (context.idsAsHex && !db.isEthereumAddress(doc.contextId))
        ) {
          console.log(`Remove invalid contextId: ${doc.contextId}`);
          contextColl.removeByExample(doc);
        } else if (
          context.idsAsHex &&
          db.isEthereumAddress(doc.contextId) &&
          doc.contextId != doc.contextId.toLowerCase()
        ) {
