...
describe('errors', function() {
  ...
  it("should not be able to 'Social Recovery' when the signers' recovery connection set less than 7 days ago", function () {
    db.connect({id1: u2.id, id2: u1.id, level: 'already known', 'timestamp': 1 });
    db.connect({id1: u1.id, id2: u2.id, level: 'recovery', 'timestamp': 1 });
    db.connect({id1: u3.id, id2: u1.id, level: 'already known', 'timestamp': 1 });
    db.connect({id1: u1.id, id2: u3.id, level: 'recovery', 'timestamp': 1 });
    db.connect({id1: u4.id, id2: u1.id, level: 'already known', 'timestamp': Date.now() });
    db.connect({id1: u1.id, id2: u4.id, level: 'recovery', 'timestamp': Date.now() });
