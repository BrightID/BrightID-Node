...
describe('operations', function() {
  ...
  describe('family groups', function() {
    before(function() {
      ...
      groupsColl.update(groupId, { 'seed': true, 'quota': 20, 'region': 'community one' });
      seedsColl.insert({
        user: u7.id,
        type: 'community',
    ...
    it('admins should be able to "Invite" users to the seed group', function() {
      const groupId = hash('randomstr1');
      const data = 'some data';
      const op = {
        'v': 6,
    ...
    it('users should be able to "Add Membership" to the seed group and become seed', function() {
      const groupId = hash('randomstr1');
      const op = {
        'v': 6,
        'name': 'Add Membership',
      ...
      const seedU8 = seedsColl.firstExample({
        'user': u8.id,
        'type': 'community',
        'group': groupId
      });
      seedU8.quota.should.equal(0);
    });

    it('seeds should be able to "Transfer Quota" to each others', function() {
      const groupId = hash('randomstr1');
      const data = 'some data';
      const op = {
        'v': 6,
      ...
      const seedU7 = seedsColl.firstExample({
        'user': u7.id,
        'type': 'community',
        'group': groupId
      });
      seedU7.quota.should.equal(15);
