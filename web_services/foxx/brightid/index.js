...
const handlers = {
  ...
  ipGet: function (req, res) {
    let ip =
      ...
      module.context.configuration.ip;
    if (ip) {
      res.send({
        data: {
          ip,
...
router
  ...
  .description(
    "Gets a user's score, verifications, joining date, lists of connections, groups and eligible groups."
  )
...
router
  .get("/verifications/:app/:contextId", handlers.verificationGet)
  .pathParam(
    "app",
  ...
  .queryParam(
    "verification",
    joi.string().description("the verification expression")
  )
  .queryParam(
    "signed",
    joi
  ...
  .description(
    "Gets a signed verification for the user that is signed by the node"
  )
...
router
  ...
  .description(
    "Gets list of all of contextIds in the context that are sponsored and verified for using an app"
  )
...
router
  .put("/testblocks/:app/:action/:contextId", handlers.testblocksPut)
  .pathParam("app", joi.string().required().description("The key of app"))
  .pathParam(
    "action",
    joi
      .string()
      .valid("sponsorship", "link", "verification")
      .required()
  ...
  .queryParam(
    "testingKey",
    joi.string().required().description("the secret key for testing the app")
  )
  .summary("Block user's verification for testing")
  .description(
    "Updating state of contextId to be considered as unsponsored, unlinked or unverified temporarily for testing"
  )
  .response(null);

router
  .delete("/testblocks/:app/:action/:contextId", handlers.testblocksDelete)
  .pathParam(
    "app",
    joi.string().required().description("Unique name of the app")
  )
  ...
  .queryParam(
    "testingKey",
    joi.string().description("the testing private key of the app")
  )
  .summary("Remove blocking state applied on user's verification for testing")
  .description(
    "Remove limitations applied to a contextId to be considered as unsponsored, unlinked or unverified temporarily for testing"
  )
  .response(null);

router
  .get("/contexts/:context/dump", handlers.contextDumpGet)
  ...
  .queryParam(
    ...
    joi
      ...
      .required()
      .description(
        "the one time passcode that authorize access to this endpoint once"
      )
  )
...
router
  ...
  .description(
    "Gets a group's admins, founders, info, isNew, region, seed, type, url, timestamp, members, invited and eligible members."
  )
  .response(schemas.groupGetResponse)
  .error(404, "Group not found");

router
  .get("/sponsorships/:contextId", handlers.sponsorshipGet)
  .pathParam(
    "contextId",
    joi
      .string()
