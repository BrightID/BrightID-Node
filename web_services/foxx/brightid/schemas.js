...
const operations = {
  ...
  'Sponsor': {
    appUserId: joi.string().required().description('the app generated id that is being sponsored'),
...
Object.keys(operations).forEach(name => {
  operations[name] = Object.assign({
    ...
    timestamp: joi.number().required().description('the timestamp (milliseconds since epoch) when the operation was created'),
  ...
  verificationsGetResponse: joi.object({
    data: joi.array().items(
      joi.object({
        ...
        appUserId: joi.string().required().description('the id of the user within the app'),
